    ...
    try:
        ...
        try:
            ...
            validation_status = {
                ...
                "error_count": 1,
                "warning_count": 0,
                "errors": [{"component": "Validation", "message": f"Validation system failed: {str(e)}"}],
                "configuration": {}
            }
        ...
        if not validation_status.get("valid", False):
            ...
            health_data["warnings"] = [f"Configuration validation failed with {validation_status.get('error_count', 0)} errors"]
        ...
        if missing_env:
            health_data["status"] = "degraded"
            health_data.setdefault("warnings", []).append(f"Missing environment variables: {', '.join(missing_env)}")
...
async def system_info() -> Dict[str, Any]:
    ...
    try:
        return {
            ...
            "directories": {
                "current": os.getcwd(),
                "outputs": os.path.abspath("./outputs") if os.path.exists("./outputs") else "not_created",
