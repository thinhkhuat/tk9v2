...
try:
    ...
    config_valid = validate_startup_configuration(
        verbose=False
    )  # Set to True for detailed output during debugging
...
def handle_provider_info():
    ...
    try:
        ...
        if provider_info.get("fallback_llm"):
            ...
            print(
                f"Fallback LLM:     {fb_llm['provider']} ({'✅' if fb_llm['has_api_key'] else '❌'})"
            )

        if provider_info.get("fallback_search"):
            fb_search = provider_info["fallback_search"]
            print(
                f"Fallback Search:  {fb_search['provider']} ({'✅' if fb_search['has_api_key'] else '❌'})"
            )
...
Examples:
  ...
  python main.py --research "AI trends"    # Run specific research query
    ...
    parser.add_argument(
        "--config", action="store_true", help="Validate configuration and show detailed status"
    )
    parser.add_argument(
        "--provider-info",
        action="store_true",
    ...
    parser.add_argument("--research", type=str, help="Run research on a specific query")
    parser.add_argument(
        "--language", type=str, default=None, help="Target language for research output"
    )
    parser.add_argument(
        "--tone",
        type=str,
    ...
    if args.research:
        ...
        research_report = await run_research_task(
            query=args.research, tone=tone_enum, language=args.language, write_to_files=True
        )
    else:
