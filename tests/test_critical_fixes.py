...
class TestImportFixes:
    ...
    def test_mcp_retriever_warning_suppression(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            ...
            mcp_warnings = [
                record for record in caplog.records if "MCPRetriever" in record.getMessage()
            ]
    ...
    def test_brave_retriever_integration(self):
        ...
        with patch.dict(
            os.environ, {"PRIMARY_SEARCH_PROVIDER": "brave", "BRAVE_API_KEY": "test_api_key"}
        ):
            try:
    ...
    async def test_error_handling_in_async_workflows(self):
        ...
        try:
            ...
            response = await provider_manager.llm_generate(
                "test prompt", fallback=True, max_retries=1
            )
...
class TestConfigurationValidation:
    ...
    def test_llm_provider_validation(self):
        ...
        with patch.dict(
            os.environ, {"PRIMARY_LLM_PROVIDER": "google_gemini", "GOOGLE_API_KEY": "test_key"}
        ):
    ...
    def test_invalid_provider_validation(self):
        ...
        with patch.dict(os.environ, {"PRIMARY_LLM_PROVIDER": "invalid_provider"}):
            result = validator.validate_llm_provider()
    ...
    def test_comprehensive_validation(self):
        ...
        with patch.dict(
            ...
            result = validator.run_comprehensive_validation(
                check_env_file=False, check_task_json=False, check_directories=False
            )
...
class TestProviderFailover:
    ...
    async def test_enhanced_llm_generation(self):
        ...
        try:
            ...
            response = await failover_integration.get_llm_response("test prompt", max_retries=1)
...
class TestErrorHandling:
    ...
    def test_missing_api_key_handling(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch.dict(
                os.environ,
                {"PRIMARY_LLM_PROVIDER": "google_gemini", "PRIMARY_SEARCH_PROVIDER": "brave"},
            ):
                validator = ConfigurationValidator()
                result = validator.validate_llm_provider()
