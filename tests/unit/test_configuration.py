...
from multi_agents.config.providers import (
    LLMConfig, SearchConfig, MultiProviderConfig, ProviderConfigManager,
...
class TestProviderConfigManager:
    ...
    def test_config_manager_fallback_without_strategy(self):
        ...
        if not validation_result["valid"]:
            assert any("strategy" in issue.lower() or "fallback" in issue.lower() 
    ...
    def test_config_manager_provider_switching(self, mock_env_vars):
        ...
        if switched_config:
            assert switched_config.primary_search.provider != original_config.primary_search.provider
            assert switched_config.primary_search.provider == original_config.fallback_search.provider
