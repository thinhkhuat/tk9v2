...
class DashboardTester:
    ...
    async def test_connectivity(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("Dashboard Connectivity", False, f"HTTP {response.status_code}", response_time)
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_fields and data["status"] == "healthy":
                    ...
                    self.log_result("Health Endpoint", False, f"Missing fields: {missing_fields}", response_time)
            else:
                self.log_result("Health Endpoint", False, f"HTTP {response.status_code}", response_time)
                ...
                if isinstance(data, list):
                    ...
                    if session_count > 0:
                        ...
                        if not missing_fields:
                            ...
                            self.log_result("Sessions API", False, f"Missing fields: {missing_fields}", response_time)
                    else:
                        self.log_result("Sessions API", True, "No sessions found (valid empty response)", response_time)
                else:
                    self.log_result("Sessions API", False, "Response is not a list", response_time)
            else:
                self.log_result("Sessions API", False, f"HTTP {response.status_code}", response_time)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "text/html" in content_type:
                    ...
                    self.log_result("Dashboard Homepage", False, f"Wrong content type: {content_type}", response_time)
            else:
                self.log_result("Dashboard Homepage", False, f"HTTP {response.status_code}", response_time)
    ...
    async def test_file_downloads(self):
        ...
        try:
            ...
            if not sessions:
                self.log_result("File Downloads", True, "No sessions available for download test (acceptable)")
                return
            ...
            if not test_session:
                self.log_result("File Downloads", True, "No sessions with files available (acceptable)")
                return
                ...
                try:
                    ...
                    if "/download/" in url:
                        ...
                        if response.status_code == 200:
                            ...
                            download_results.append(f"{file_info['filename']}: OK ({content_length} bytes)")
                        else:
                            download_results.append(f"{file_info['filename']}: HTTP {response.status_code}")
    ...
    async def test_performance(self):
        ...
        try:
            ...
            performance_results.append(f"Concurrent (5x): {success_count}/5 OK in {concurrent_time:.3f}s")
            
        except Exception as e:
            performance_results.append("Concurrent: ERROR")
        
        details = "; ".join(performance_results)
        all_good = all("SLOW" not in result and "ERROR" not in result for result in performance_results)
