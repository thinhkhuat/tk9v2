            ...
            if len(url_parts) >= 4 and url_parts[1] == "download":
                ...
                if response.status_code == 200:
                    ...
                    if file_ext in expected_mime_types:
                        ...
                        assert expected_mime in content_type, f"Wrong MIME type for {filename}: {content_type}"
...
class TestSecurity(TestDashboardValidation):
    ...
    async def test_invalid_filename_protection(self, client):
        ...
        for invalid_filename in invalid_filenames:
            ...
            assert response.status_code in [400, 404], f"Invalid filename not blocked: {invalid_filename}"
...
class TestPerformance(TestDashboardValidation):
    ...
    async def test_concurrent_requests(self, client):
        ...
        print(f"✅ Concurrent requests handled: 10 requests in {total_time:.2f}s (avg: {avg_time:.2f}s)")
...
class TestDataIntegrity(TestDashboardValidation):
    ...
    async def test_session_data_structure(self, client):
        ...
        for session in sessions:
            ...
            assert session["session_id"].startswith("run_"), f"Invalid session ID format: {session['session_id']}"
            
            # Validate file count matches actual files array length
            assert len(session["files"]) == session["file_count"], f"File count mismatch in session {session['session_id']}"
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url="http://0.0.0.0:12656", timeout=10.0) as client:
            try:
                response = await client.get("/api/health")
                if response.status_code == 200:
                    reporter.add_result("api_endpoints", "connectivity", "PASS", 
                                      {"response_time": "< 1s", "status_code": 200})
                    print("✅ Dashboard connectivity confirmed")
                else:
                    reporter.add_result("api_endpoints", "connectivity", "FAIL",
