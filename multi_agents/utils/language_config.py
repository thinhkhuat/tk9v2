...
class LanguageConfig:
    ...
    def get_prompt_prefix(cls, lang_code: Optional[str] = None) -> str:
        ...
        if lang_code == "en":
            ...
            return "Bạn PHẢI trả lời và làm việc hoàn toàn bằng tiếng Việt. All content, research, analysis, and output must be in Vietnamese language."
        elif lang_code == "es":
            return "Debes responder y trabajar completamente en español. Todo el contenido, investigación, análisis y salida debe estar en idioma español."
        elif lang_code == "fr":
            return "Vous DEVEZ répondre et travailler entièrement en français. Tout le contenu, la recherche, l'analyse et la sortie doivent être en langue française."
        elif lang_code == "de":
            return "Sie MÜSSEN vollständig auf Deutsch antworten und arbeiten. Alle Inhalte, Recherchen, Analysen und Ausgaben müssen in deutscher Sprache sein."
        elif lang_code == "zh":
            return "您必须完全用中文回答和工作。所有内容、研究、分析和输出都必须使用中文。"
        elif lang_code == "ja":
            return "日本語で完全に回答し、作業してください。すべてのコンテンツ、研究、分析、出力は日本語である必要があります。"
        elif lang_code == "ko":
            return "한국어로 완전히 응답하고 작업해야 합니다. 모든 콘텐츠, 연구, 분석 및 출력은 한국어여야 합니다."
        else:
    ...
    def get_search_terms_instruction(cls, lang_code: Optional[str] = None) -> str:
        ...
        if lang_code == "en":
            ...
            return (
                "Générez des termes de recherche en français. Use French keywords for web searches."
            )
        elif lang_code == "de":
    ...
    def get_status_message(cls) -> str:
        ...
        return (
            f"Research Language: {language_name} ({lang_code.upper()}) | Search Region: {country}"
        )
