...
class DraftManager:
    ...
    def save_agent_output(self, 
        ...
        Args:
            ...
            step: Optional step within the phase
        ...
        # Create filename
        filename = f"{timestamp}_{agent_name}{step_suffix}.{ext}"
    ...
    def save_research_state(self, 
        ...
        filename = f"{timestamp}_research_state{step_suffix}.json"
        phase_dir = self._get_phase_directory(phase)
        filepath = os.path.join(phase_dir, filename)
        ...
        # Update metadata
        self._update_metadata("system", phase, filepath, step, {
            "type": "research_state",
            "keys": list(research_state.keys())
        })
        
        return filepath
    
    def save_intermediate_draft(self, 
                               content: str, 
                               phase: str, 
                               section: Optional[str] = None,
                               agent: str = "writer") -> str:
        """
        ...
        Args:
            ...
            section: Optional section name being worked on
        ...
        """
        timestamp = datetime.now().strftime("%H%M%S")
        section_suffix = f"_{sanitize_filename(section)}" if section else ""
        
        filename = f"{timestamp}_{agent}_draft{section_suffix}.md"
        phase_dir = self._get_phase_directory(phase)
        filepath = os.path.join(phase_dir, filename)
        ...
        self._update_metadata(agent, phase, filepath, section, {
            "type": "intermediate_draft",
    ...
    def save_human_feedback(self, 
        ...
        # Update metadata
        self._update_metadata("human", "feedback", filepath, None, {
            "type": "human_feedback"
        })
    ...
    def save_revision_history(self, 
        ...
        # Update metadata
        self._update_metadata(agent, "revisions", filepath, None, {
    ...
    def create_phase_summary(self, phase: str) -> str:
        ...
        if phase_history:
            ...
            for output in phase_history:
                summary_content += f"- **{output['agent']}** ({output['timestamp']}): {output['filepath']}\n"
                if output.get('step'):
