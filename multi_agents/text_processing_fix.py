...
class TextChunkingFix:
    ...
    def safe_text_split(self, text: str, chunk_size: int = None, chunk_overlap: int = None) -> List[str]:
        """
                    ...
                    try:
                        ...
                        for doc in self.documents:
                            if hasattr(doc, 'page_content'):
                                is_valid, cleaned_content = text_fixer.validate_text_for_processing(doc.page_content)
                                if is_valid:
                        ...
                        try:
                            result = await super().async_get_context(cleaned_query, max_results, cost_callback)
                            return result
                        finally:
