...
class ProviderFactory:
    ...
    def create_llm_provider(cls, config: LLMConfig, validate: bool = True) -> BaseLLMProvider:
        ...
        if not provider_class:
            ...
            raise ValueError(f"LLM provider {config.provider.value} not implemented in abstraction layer")
    ...
    @classmethod
    def create_search_provider(cls, config: SearchConfig, validate: bool = True) -> BaseSearchProvider:
        """
        ...
        if not provider_class:
            raise ValueError(f"Search provider {config.provider.value} not implemented in abstraction layer")
    ...
    def _validate_search_config(cls, config: SearchConfig):
        ...
        providers_requiring_api_key = [SearchProvider.TAVILY, SearchProvider.BRAVE, SearchProvider.GOOGLE, SearchProvider.SERPAPI]
        ...
        if config.search_depth not in valid_depths:
            issues.append(f"Search depth '{config.search_depth}' not in valid options: {valid_depths}")
    ...
    @classmethod
    def create_provider_manager(cls, config_manager: ProviderConfigManager, validate: bool = True) -> ProviderManager:
        """
        ...
        if config_manager.config.fallback_llm:
            try:
                fallback_llm = cls.create_llm_provider(config_manager.get_llm_config(prefer_fallback=True), validate=validate)
        ...
        if config_manager.config.fallback_search:
            try:
                fallback_search = cls.create_search_provider(config_manager.get_search_config(prefer_fallback=True), validate=validate)
