...
from ..enhanced_base import (
    EnhancedBaseSearchProvider, SearchResult, SearchResponse, SearchProviderError, ProviderHealth
)
...
class EnhancedBraveSearchProvider(EnhancedBaseSearchProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Enhanced Brave search provider initialized with {self.requests_per_minute} RPM limit")
    ...
    async def _perform_search(self, search_type: str, query: str, **kwargs) -> SearchResponse:
                    ...
                    async with session.get(endpoint, params=params, headers=headers) as response:
                        ...
                        if response.status == 200:
                            ...
                            if attempt < self.max_retries_per_request - 1:
                                ...
                                raise SearchProviderError(error_msg, "brave", "rate_limit", retry_after)
                            ...
                            logger.warning(f"Brave server error on attempt {attempt + 1}: {error_msg}")
                            last_error = SearchProviderError(error_msg, "brave", f"server_error_{response.status}")
        ...
        logger.error(f"Brave {search_type} search failed after {self.max_retries_per_request} attempts")
        raise last_error if last_error else SearchProviderError("All retry attempts failed", "brave")
    ...
    def _truncate_query(self, query: str, max_length: int = 400, max_words: int = 50) -> str:
        ...
        logger.info(f"Query truncated: {original_word_count} words → {final_word_count} words, {original_length} chars → {len(truncated)} chars")
        return truncated
            ...
            try:
                ...
                result = SearchResult(
                    ...
                    metadata={
                        "type": "news",
                        "source": source_name,
                        "source_url": meta_url.get("scheme", "") + "://" + meta_url.get("hostname", ""),
                        "language": item.get("language"),
    ...
    async def _check_rate_limit(self):
        ...
        if self.daily_request_count >= self.daily_request_limit:
            ...
            if wait_until_reset > 0:
                logger.warning(f"Daily request limit reached, waiting {wait_until_reset/3600:.1f} hours")
    ...
    def _get_rate_limit_remaining(self) -> Dict[str, int]:
        ...
        return {
            ...
            "daily_reset_in_seconds": int(self.daily_reset_time - now) if self.daily_reset_time else 0
        }
