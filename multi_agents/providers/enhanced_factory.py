...
from ..config.providers import ProviderConfigManager, LLMProvider, SearchProvider, LLMConfig, SearchConfig
from .enhanced_base import (
    EnhancedProviderManager, EnhancedBaseLLMProvider, EnhancedBaseSearchProvider,
    FailoverEvent, ProviderHealth, FailoverReason
)
...
class EnhancedProviderFactory:
    ...
    @classmethod
    def create_llm_provider(cls, config: LLMConfig, validate: bool = True) -> EnhancedBaseLLMProvider:
        """
    ...
    @classmethod
    def create_search_provider(cls, config: SearchConfig, validate: bool = True) -> EnhancedBaseSearchProvider:
        """
    ...
    @classmethod
    def _wrap_basic_llm_provider(cls, basic_provider, config: LLMConfig) -> 'WrappedBasicLLMProvider':
        """Wrap a basic LLM provider to work with enhanced manager"""
        return WrappedBasicLLMProvider(basic_provider, config)
    
    @classmethod
    def _wrap_basic_search_provider(cls, basic_provider, config: SearchConfig) -> 'WrappedBasicSearchProvider':
    ...
    @classmethod
    async def create_provider_manager(cls, config_manager: ProviderConfigManager, 
                                    validate: bool = True, enable_monitoring: bool = True) -> EnhancedProviderManager:
        """
        ...
        try:
            primary_llm = cls.create_llm_provider(config_manager.get_llm_config(), validate=validate)
            manager.register_llm_provider("primary", primary_llm, is_primary=True)
            primary_llm_registered = True
            logger.info(f"Primary LLM provider registered: {config_manager.get_llm_config().provider.value}")
        ...
        try:
            primary_search = cls.create_search_provider(config_manager.get_search_config(), validate=validate)
            manager.register_search_provider("primary", primary_search, is_primary=True)
            primary_search_registered = True
            logger.info(f"Primary search provider registered: {config_manager.get_search_config().provider.value}")
        except Exception as e:
        ...
        if config_manager.config.fallback_llm and primary_llm_registered:
            try:
                ...
                logger.info(f"Fallback LLM provider registered: {config_manager.config.fallback_llm.provider.value}")
            except Exception as e:
        ...
        if config_manager.config.fallback_search and primary_search_registered:
            try:
                ...
                logger.info(f"Fallback search provider registered: {config_manager.config.fallback_search.provider.value}")
            except Exception as e:
...
class EnhancedGPTResearcherConfig:
    ...
    def __init__(self, config_manager: ProviderConfigManager, provider_manager: EnhancedProviderManager = None):
