...
class EnhancedBaseLLMProvider(ABC):
    ...
    async def health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            if response.success:
                ...
                status = (ProviderHealth.UNHEALTHY if self._consecutive_failures >= self.max_consecutive_failures 
    ...
    async def is_healthy(self, force_check: bool = False) -> bool:
        ...
        if (force_check or not self._last_health_check or 
...
class EnhancedBaseSearchProvider(ABC):
    ...
    async def health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            test_query = "test"
            response = await asyncio.wait_for(
                self.search(test_query, max_results=1),
                timeout=self.health_check_timeout
            )
...
class ProviderError(Exception):
    ...
    def __init__(self, message: str, provider: str, error_code: str = None, retry_after: Optional[int] = None):
...
class EnhancedProviderManager:
    ...
    def register_llm_provider(self, name: str, provider: EnhancedBaseLLMProvider, 
    ...
    def register_search_provider(self, name: str, provider: EnhancedBaseSearchProvider,
                                is_primary: bool = False):
    ...
    def _start_health_monitoring(self, provider: Union[EnhancedBaseLLMProvider, EnhancedBaseSearchProvider], 
        ...
        async def monitor():
            while self._monitoring_enabled:
                try:
                    ...
                    logger.error(f"Health monitoring failed for {provider_type} provider {name}: {e}")
    ...
    async def llm_generate(self, prompt: str, provider_name: str = None, 
                          fallback: bool = True, max_retries: int = 3, **kwargs) -> LLMResponse:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if self._active_llm_provider != provider_name:
                        await self._perform_llm_failover(self._active_llm_provider, provider_name, 
                    ...
                    if attempt == max_retries - 1:
                        ...
                        logger.error(f"LLM provider {provider_name} failed after {max_retries} attempts: {e}")
                        if fallback and len(providers) > 1:
                            # Try failover to next provider
                            await self._perform_llm_failover(provider_name, providers[providers.index(provider_name) + 1], 
                                                           FailoverReason.API_ERROR, str(e))
                            break
                        else:
                            raise LLMProviderError(str(e), provider_name)
        
        # All providers failed
        raise LLMProviderError(f"All LLM providers failed after {retry_count} attempts. Last error: {last_error}", "all")
    
    async def search_query(self, query: str, provider_name: str = None, 
                          search_type: str = "web", fallback: bool = True, 
                          max_retries: int = 3, **kwargs) -> SearchResponse:
        ...
        for provider_name in providers:
            ...
            # Check provider health before using
            if not await provider.is_healthy():
                logger.warning(f"Search provider {provider_name} is not healthy, trying next provider")
                if fallback and len(providers) > 1:
                    continue
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if self._active_search_provider != provider_name:
                        await self._perform_search_failover(self._active_search_provider, provider_name, 
                                                          FailoverReason.MANUAL_SWITCH)
                    ...
                    if attempt == max_retries - 1:
                        # Max retries reached for this provider
                        logger.error(f"Search provider {provider_name} failed after {max_retries} attempts: {e}")
                        if fallback and len(providers) > 1:
                            # Try failover to next provider
                            next_idx = providers.index(provider_name) + 1
                            if next_idx < len(providers):
                                await self._perform_search_failover(provider_name, providers[next_idx], 
                                                                   FailoverReason.API_ERROR, str(e))
                            break
                        else:
        ...
        # All providers failed
        raise SearchProviderError(f"All search providers failed after {retry_count} attempts. Last error: {last_error}", "all")
    ...
    def _get_llm_provider_order(self, preferred: str = None) -> List[str]:
        ...
        if preferred and preferred in self.llm_providers:
            ...
            remaining = [name for name in self.llm_providers.keys() if name != self._active_llm_provider]
    ...
    def _get_search_provider_order(self, preferred: str = None) -> List[str]:
        ...
        if preferred and preferred in self.search_providers:
            ...
            # Add remaining providers sorted by success rate
            remaining = [name for name in self.search_providers.keys() if name != self._active_search_provider]
            remaining.sort(key=lambda x: self.usage_stats["search"][x]["success_rate"], reverse=True)
            providers.extend(remaining)
        
        return providers
    
    async def _perform_llm_failover(self, from_provider: str, to_provider: str, 
                                   reason: FailoverReason, error_message: str = None):
    ...
    async def _perform_search_failover(self, from_provider: str, to_provider: str,
                                      reason: FailoverReason, error_message: str = None):
    ...
    def _update_llm_stats(self, provider_name: str, response: Optional[LLMResponse], 
    ...
    def _update_search_stats(self, provider_name: str, response: Optional[SearchResponse],
                           latency: float, success: bool):
    ...
    async def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        for name, provider in self.llm_providers.items():
            ...
            llm_status[name] = {
                "available": health_result.status in [ProviderHealth.HEALTHY, ProviderHealth.DEGRADED],
    ...
    async def force_failover(self, provider_type: str, to_provider: str):
        ...
        if provider_type == "llm" and to_provider in self.llm_providers:
            await self._perform_llm_failover(self._active_llm_provider, to_provider, 
                                           FailoverReason.MANUAL_SWITCH)
        elif provider_type == "search" and to_provider in self.search_providers:
            await self._perform_search_failover(self._active_search_provider, to_provider,
                                              FailoverReason.MANUAL_SWITCH)
        else:
            raise ValueError(f"Invalid provider type or provider name: {provider_type}/{to_provider}")
    
    async def cleanup(self):
        """Cleanup resources"""
        self.disable_monitoring()
        await asyncio.gather(*[task for task in self._health_check_tasks.values()], 
