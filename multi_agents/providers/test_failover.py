...
from .enhanced_base import (
    EnhancedProviderManager, ProviderHealth, FailoverReason, 
    LLMProviderError, SearchProviderError
)
from .enhanced_factory import EnhancedProviderFactory
    ...
    try:
        ...
        async with managed_failover_system(enable_monitoring=False) as (integration, success):
            ...
            if success:
                ...
                logger.info(f"Provider status obtained: {len(status.get('llm_providers', {})) + len(status.get('search_providers', {}))}")
                
                # Test 3: Health checks
                health_results = await integration.health_check_all_providers()
                logger.info(f"Health check completed for {len(health_results.get('llm_providers', {})) + len(health_results.get('search_providers', {}))}")
