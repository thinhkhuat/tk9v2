...
from .date_context import get_current_date_context, format_system_prompt_with_date
...
async def call_model(
    ...
    for i, msg in enumerate(enhanced_prompt):
        if msg.get("role") == "system":
            ...
            if "current year" not in original_content.lower() and "today's date" not in original_content.lower():
        ...
        if response_format == "json":
            ...
            def extract_json_from_response(text):
                ...
                try:
                    ...
                    fixed = re.sub(r'([{,]\s*)(\w+):', r'\1"\2":', fixed)  # Add quotes to unquoted keys
            ...
            try:
                ...
                logger.error(f"JSON parsing failed for response preview: {response[:500] if response else 'None'}...")
