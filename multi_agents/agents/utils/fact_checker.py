...
class FactCheckResult:
    ...
    def can_block_workflow(self) -> bool:
        ...
        """
        return (
            self.status == VerificationStatus.VERIFIED_FALSE 
...
class FactChecker:
    ...
    def __init__(self, api_key: str = None):
        ...
        self.factual_claim_indicators = [
            "incorrect", "wrong", "false", "inaccurate", "mistaken",
            "error", "not true", "actually", "in fact", "really",
            "should be", "must be", "is not", "was not", "will not"
        ]
        
        # Keywords that indicate style/structure issues (not factual)
        self.style_indicators = [
            "formatting", "structure", "clarity", "grammar", "spelling",
            "readability", "flow", "organization", "style", "tone",
            "coherence", "consistency", "redundant", "verbose"
        ]
        ...
        self.content_patterns = {
            'dates': ['year', 'date', 'january', 'february', 'march', 'april', 'may', 'june',
    ...
    def _safe_lower(self, text: Any) -> str:
        ...
        try:
            ...
            if not isinstance(safe_text, str):
                logger.warning(f"_safe_string returned non-string type {type(safe_text)}, converting again")
    ...
    def categorize_feedback(self, feedback_input: Any) -> Tuple[FeedbackCategory, Dict[str, Any]]:
        ...
        try:
            if isinstance(feedback_input, dict):
                ...
                possible_text_keys = [
                    'feedback', 'review', 'content', 'text', 'message', 
                    'comments', 'notes', 'analysis', 'response'
                ]
        ...
        try:
            formatting_phrase_count = sum(1 for phrase in strong_formatting_phrases 
        ...
        if formatting_phrase_count >= 2:
            ...
            analysis['reasoning'].append(f"Strong formatting terminology detected ({formatting_phrase_count} phrases)")
        else:
                ...
                for pattern in patterns:
                    try:
                        ...
                        if pattern_str and feedback_lower and pattern_str in feedback_lower:
                            ...
                            analysis['detected_patterns']['formatting'].append(f"{category}: {pattern_str}")
                    except Exception as e:
                ...
                for pattern in patterns:
                    try:
                        ...
                        if pattern_str and feedback_lower and pattern_str in feedback_lower:
                            ...
                            analysis['detected_patterns']['content'].append(f"{category}: {pattern_str}")
                    except Exception as e:
                ...
                for pattern in patterns:
                    try:
                        ...
                        if pattern_str and feedback_lower and pattern_str in feedback_lower:
                            ...
                            analysis['detected_patterns']['hybrid'].append(f"{category}: {pattern_str}")
                    except Exception as e:
                ...
                try:
                    if indicator_str in feedback_lower:
                        ...
                        try:
                            ...
                            context_around = ' '.join(context_words) if context_words else feedback_lower
                            ...
                            in_formatting_context = any(phrase in context_around 
                            ...
                            if in_formatting_context:
                                ...
                                analysis['detected_patterns']['hybrid'].append(f"factual_in_format_context: {indicator_str}")
                            else:
                                analysis['content_score'] += 3.0  # Strong content indicator
                                analysis['detected_patterns']['content'].append(f"factual_claim: {indicator_str}")
                        except Exception as e:
                            logger.warning(f"Error analyzing context for indicator '{indicator_str}': {e}")
        ...
        content_change_patterns = [
            ...
            (r'\b(?:date|year|time|when)\s+(?:should be|is actually|was really)\s+([^.,]+)', 'date_change'),
            (r'\b(?:name|person|president|ceo|founder)\s+(?:should be|is actually)\s+([^.,]+)', 'name_change'),
            (r'\b(?:number|amount|percentage|statistics?)\s+(?:should be|is actually)\s+([^.,]+)', 'number_change'),
            (r'\b(?:not|isn\'t|wasn\'t|weren\'t)\s+([^.,]+)', 'negation'),
        ]
        
        formatting_exclusions = [
            'grammar', 'spelling', 'format', 'formatting', 'style', 'structure',
            'capitalization', 'punctuation', 'organization', 'flow', 'readability'
        ]
                ...
                try:
                    ...
                    for match in matches:
                        try:
                            ...
                            is_formatting_related = any(exclusion in match_lower 
                                                      for exclusion in formatting_exclusions
                                                      if exclusion and match_lower)
                            
                            if is_formatting_related:
                                analysis['hybrid_score'] += 1.0
                                analysis['detected_patterns']['hybrid'].append(f"format_related_{change_type}: {match}")
                            else:
                                analysis['content_score'] += 2.0
                                analysis['detected_patterns']['content'].append(f"semantic_change: {match}")
                        except Exception as e:
            ...
            for pattern in disguised_content_patterns:
                try:
                    if feedback_lower and re.search(pattern, feedback_lower):
                        ...
                        analysis['detected_patterns']['hybrid'].append(f"disguised_content: {pattern}")
                except Exception as e:
        ...
        try:
            ...
            if total_score == 0:
                ...
                analysis['reasoning'].append("No categorization patterns detected - defaulting to CONTENT for safety")
            ...
            analysis['reasoning'].append(f"Error in ratio calculation - defaulting to CONTENT for safety: {e}")
            return FeedbackCategory.CONTENT, analysis
        ...
        if analysis.get('formatting_bias', False):
            ...
            if format_ratio >= 0.4 and content_ratio < 0.6:
                analysis['reasoning'].append(f"Strong formatting bias overrides content signals (F:{format_ratio:.2f}, C:{content_ratio:.2f})")
                return FeedbackCategory.FORMATTING, analysis
            elif content_ratio >= 0.6:
                # Still predominantly content despite formatting terms
                analysis['reasoning'].append(f"Content ratio dominates despite formatting bias: {content_ratio:.2f}")
                return FeedbackCategory.HYBRID, analysis
        
        if format_ratio >= 0.6 and content_ratio < 0.3:
            # Predominantly formatting feedback
            analysis['reasoning'].append(f"Predominantly formatting feedback (ratio: {format_ratio:.2f})")
            ...
            analysis['reasoning'].append(f"Predominantly content feedback (ratio: {content_ratio:.2f})")
            ...
            analysis['reasoning'].append(f"Mixed or hybrid patterns (H:{hybrid_ratio:.2f}, C:{content_ratio:.2f}, F:{format_ratio:.2f})")
            ...
            analysis['reasoning'].append(f"High absolute content score: {analysis['content_score']}")
            ...
            analysis['reasoning'].append(f"Formatting score dominates: {analysis['formatting_score']} vs {analysis['content_score']}")
            return FeedbackCategory.FORMATTING, analysis
        
        else:
            # Default based on context
            if analysis.get('formatting_bias', False):
                analysis['reasoning'].append("Formatting bias detected - defaulting to HYBRID for careful processing")
                return FeedbackCategory.HYBRID, analysis
            else:
                analysis['reasoning'].append("Ambiguous categorization - defaulting to CONTENT for safety")
                analysis['reasoning'].append(f"Scores: F={analysis['formatting_score']}, C={analysis['content_score']}, H={analysis['hybrid_score']}")
                return FeedbackCategory.CONTENT, analysis
        ...
        try:
            ...
            try:
                if query_parts:
                    ...
                    unique_parts = list(set(part.strip() for part in query_parts if part and part.strip()))
                    if unique_parts:
                ...
                if response.status_code == 200:
                    ...
                    for item in web_results:
                        ...
                        try:
                            ...
                            result_item = {
                                "title": self._safe_string(self._safe_dict_get(item, "title", default="")),
                                "url": self._safe_string(self._safe_dict_get(item, "url", default="")),
                                "description": self._safe_string(self._safe_dict_get(item, "description", default="")),
                                "age": self._safe_string(self._safe_dict_get(item, "age", default="")),
                                "domain": self._safe_string(self._safe_dict_get(item, "meta_url", "hostname", default=""))
                            }
                    ...
                    logger.info(f"BRAVE search successful: {len(formatted_results)} results for query '{safe_query[:50]}...'")
                    ...
                    logger.error(f"BRAVE API HTTP error: {response.status_code} - {response.text[:200]}")
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                    return []
    ...
    def _analyze_search_results(
        ...
        for result in search_results:
            ...
            try:
                ...
                description = self._safe_string(self._safe_dict_get(result, "description", default=""))
                ...
                try:
                    is_contradiction = any(term in title_desc for term in contradiction_terms if term)
                ...
                try:
                    evidence_entry = {
                        "source": self._safe_string(self._safe_dict_get(result, "domain", default="")),
                        "title": title,
