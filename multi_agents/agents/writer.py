...
from .utils.type_safety import (
    ensure_dict, safe_dict_get, ensure_agent_return_dict,
    ensure_list, safe_json_parse
)
...
class WriterAgent:
    ...
    async def write_sections(self, research_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if title in ["Unknown Topic", "Untitled Research Project", "Research Report", None, ""]:
            ...
            print_agent_output(f"Using original query instead of generic title: {original_query}", agent="WRITER")
        else:
            query = title
        ...
        if not data or len(data) == 0:
            print_agent_output(f"Warning: No research data available, instructing AI to focus on query: {query}", agent="WRITER")
        ...
        if isinstance(response, dict):
            ...
            for field in required_fields:
                if field in response and response[field]:
                    ...
                    if field == "table_of_contents":
                        fixed_response[field] = "Table of contents could not be generated due to parsing error"
                    elif field == "introduction":
                        fixed_response[field] = "Introduction could not be generated due to incomplete response"
                    elif field == "conclusion":
                        fixed_response[field] = "Conclusion could not be generated due to parsing error"
                    elif field == "sources":
            ...
            response = {
                ...
                "introduction": response_str[:500] + "..." if len(response_str) > 500 else response_str,
...
""",
        ...
        if isinstance(response, str):
            print_agent_output(f"Headers revision JSON parsing failed, returning original headers", agent="WRITER")
            response = headers  # Use original headers as fallback
    ...
    async def run(self, research_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(research_layout_content, dict):
            print_agent_output(f"Write sections returned invalid type, attempting intelligent content extraction", agent="WRITER")
            research_layout_content = self._extract_content_from_failed_json(str(research_layout_content), research_state)
        ...
        if research_state.get("task").get("follow_guidelines"):
            ...
            if isinstance(headers_result, dict):
                ...
                print_agent_output(f"Headers revision failed, using original headers", agent="WRITER")
            ...
            if isinstance(data, list):
                ...
                for i, item in enumerate(data):
                    if isinstance(item, dict):
                        ...
                        if content:
                            section_info += f"Content: {content[:1000]}{'...' if len(content) > 1000 else ''}\n"
    ...
    def _extract_content_from_failed_json(self, content_str: str, research_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            sections = re.split(r'\n(?=##|\*\*|Introduction|Conclusion|Table|References)', content_str, flags=re.IGNORECASE)
            
            for section in sections:
                section_str = str(section) if section else ""
                section_lower = section_str.lower().strip()
                if any(keyword in section_lower for keyword in ['introduction', 'intro', 'overview']):
                    introduction = section.strip()
                elif any(keyword in section_lower for keyword in ['conclusion', 'summary', 'findings']):
            ...
            urls = re.findall(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', content_str)
            ...
            return {
                ...
                "introduction": introduction if introduction else f"This report analyzes {research_state.get('topic', 'the research topic')} based on comprehensive research findings.",
                "conclusion": conclusion if conclusion else "Further research and analysis would provide additional insights into this topic.",
                "sources": sources
            }
            ...
            return {
                ...
                "introduction": content_str[:500] + "..." if len(content_str) > 500 else content_str,
