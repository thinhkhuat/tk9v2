...
from .utils.type_safety import (
    ensure_dict, safe_dict_get, safe_string_operation, 
    validate_file_path_operation, ensure_list, safe_list_operation
)
...
class TranslatorAgent:
    ...
    def __init__(self, websocket=None, stream_output=None, headers=None, draft_manager=None, output_dir=None):
    ...
    async def translate_report_file(self, research_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if target_lang_str.lower() == "en":
            if self.websocket and self.stream_output:
                ...
                print_agent_output("Target language is English, skipping translation...", agent="TRANSLATOR")
        ...
        try:
            ...
            markdown_files = []
            for f in all_files:
                if (safe_string_operation(f, 'endswith', '.md') and 
                    not safe_string_operation(f, 'startswith', 'WORKFLOW')):
        ...
        print_agent_output(f"Found markdown file to translate: {markdown_file_path}", agent="TRANSLATOR")
    ...
    async def run(self, research_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.draft_manager:
            self.draft_manager.save_research_state("translation", {
                **research_state,
        ...
        if translation_result and isinstance(translation_result, dict):
            ...
            if status == "success":
                ...
                print_agent_output(f"Translation completed successfully. Created {len(translated_files)} files.", agent="TRANSLATOR")
                print_agent_output("Workflow complete - all files saved in both English and translated versions.", agent="TRANSLATOR")
            elif status == "skipped":
                print_agent_output("Translation skipped (English target). Workflow complete.", agent="TRANSLATOR")
            else:
                print_agent_output(f"Translation status: {status}. Workflow complete.", agent="TRANSLATOR")
    ...
    def _find_all_format_files(self, markdown_file_path: str) -> Dict[str, str]:
        ...
        try:
            for filename in os.listdir(directory):
                ...
                if (safe_string_operation(filename, 'startswith', 'WORKFLOW') or 
                    safe_string_operation(filename, 'startswith', '.') or 
                    safe_string_operation(filename, 'startswith', '~$')):
                    continue  # Skip workflow files, hidden files, and temp files
                    
                full_path = os.path.join(directory, filename)
                if os.path.isfile(full_path):
                    # Type-safe file extension checks
                    if (safe_string_operation(filename, 'endswith', '.md') and 
                        not safe_string_operation(filename, 'endswith', '_vi.md')):
                        format_files['md'] = full_path
                    elif (safe_string_operation(filename, 'endswith', '.pdf') and 
                          not safe_string_operation(filename, 'endswith', '_vi.pdf')):
                        format_files['pdf'] = full_path
                    elif (safe_string_operation(filename, 'endswith', '.docx') and 
                          not safe_string_operation(filename, 'endswith', '_vi.docx')):
    ...
    async def _create_translated_files(self, translated_content: str, original_files: Dict[str, str], 
                                     target_language: str, language_name: str) -> List[str]:
                ...
                if md_path:
                    ...
                    if 'pdf' in original_files:
                        ...
                        if pdf_path:
                            ...
                            if os.path.exists(pdf_path):
                                ...
                                print_agent_output(f"PDF renamed to: {translated_pdf_path}", agent="TRANSLATOR")
                            else:
                                print_agent_output(f"PDF file not found at path: {pdf_path}", agent="TRANSLATOR")
                        else:
                            print_agent_output("PDF creation failed - no path returned", agent="TRANSLATOR")
                    ...
                    if 'docx' in original_files:
                        ...
                        if docx_path:
                            ...
                            print_agent_output(f"DOCX path returned: {docx_path}", agent="TRANSLATOR")
                            ...
                            if os.path.exists(docx_path):
                                ...
                                print_agent_output(f"DOCX renamed to: {translated_docx_path}", agent="TRANSLATOR")
                            else:
                                print_agent_output(f"DOCX file not found at path: {docx_path}", agent="TRANSLATOR")
                        else:
                            print_agent_output("DOCX creation failed - no path returned", agent="TRANSLATOR")
    ...
    async def _translate_markdown_content(self, content: str, language_name: str, target_language: str, task: Dict[str, Any]) -> Optional[str]:
        """
        ...
        print_agent_output(f"Translating complete document ({len(content)} chars) to {language_name}...", agent="TRANSLATOR")
        
        translated_content = await self._translate_chunk_with_retry(content, language_name, target_language, task)
        
        if translated_content:
            print_agent_output(f"Translation successful: {len(translated_content)} chars", agent="TRANSLATOR")
            
            # Return translated content as-is (markdown formatting is now preserved by endpoint)
            print_agent_output(f"Translation returned with preserved formatting: {len(translated_content)} chars", agent="TRANSLATOR")
            
            return translated_content
        else:
            print_agent_output("Translation failed - returning original content", agent="TRANSLATOR")
            return content  # Fallback to original if translation fails

    async def _translate_chunk_with_retry(self, content: str, language_name: str, target_language: str, task: Dict[str, Any], max_retries: int = 2) -> Optional[str]:
        """Translate using concurrent requests to all endpoints with intelligent result selection"""
        import asyncio
        
        print_agent_output(f"Starting concurrent translation to all endpoints ({len(content)} chars)...", agent="TRANSLATOR")
        
        for attempt in range(max_retries):
            try:
                print_agent_output(f"Concurrent translation attempt {attempt + 1}/{max_retries}", agent="TRANSLATOR")
                
                result = await self._translate_chunk_concurrent(content, language_name, target_language, task)
                if result:
                    print_agent_output(f"Concurrent translation successful on attempt {attempt + 1}", agent="TRANSLATOR")
                    return result
                else:
                    print_agent_output(f"Concurrent translation attempt {attempt + 1} failed (no valid results)", agent="TRANSLATOR")
            except Exception as e:
                print_agent_output(f"Concurrent translation attempt {attempt + 1} failed with error: {e}", agent="TRANSLATOR")
            
            if attempt < max_retries - 1:
                print_agent_output(f"Retrying concurrent translation... (attempt {attempt + 2}/{max_retries})", agent="TRANSLATOR")
        
        print_agent_output(f"All {max_retries} concurrent translation attempts failed", agent="TRANSLATOR")
        return None

    async def _translate_chunk_concurrent(self, content: str, language_name: str, target_language: str, task: dict) -> str:
        """
        ...
        for endpoint in endpoints:
            ...
            if failure_count < 3 or endpoint["priority"] == 1:  # Always include primary endpoint
                ...
                print_agent_output(f"Skipping {endpoint['name']} - too many failures this session ({failure_count})", agent="TRANSLATOR")
        ...
        if completed_results:
            selected_result = self._select_best_translation_result(completed_results, original_length)
            if selected_result:
                endpoint_name = selected_result["endpoint"]["name"]
                text_length = selected_result.get("length", 0)
                percentage = int((text_length / original_length) * 100) if original_length > 0 else 0
                print_agent_output(f"Final selection: {endpoint_name} with {text_length} chars ({percentage}% of original)", agent="TRANSLATOR")
                return selected_result.get("text")
        
        print_agent_output("No valid translation results from any endpoint", agent="TRANSLATOR")
        return None
    
    async def _wait_for_higher_priority(self, pending_tasks, task_to_endpoint, current_priority, completed_results, min_valid_length):
        """
        ...
        higher_priority_tasks = [
            ...
            if task_to_endpoint[task]['priority'] < current_priority
        ]
        ...
        for task in done:
            try:
                ...
                if result and result.get("success") and result['length'] >= min_valid_length:
                    ...
                    print_agent_output(f"Higher priority endpoint {endpoint_info['name']} completed with valid result", agent="TRANSLATOR")
                    completed_results.append({
                        **result,
                        "endpoint": endpoint_info,
                        "priority": endpoint_info["priority"]
    ...
    async def _translate_single_endpoint(self, endpoint: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if endpoint["priority"] <= 2:  # Only log for Primary and Backup-1
                print_agent_output(f"Starting translation request to {endpoint_name}: {endpoint_url} (timeout: {translation_timeout}s)", agent="TRANSLATOR")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    timeout=aiohttp.ClientTimeout(total=translation_timeout)  # Configurable timeout
                ) as response:
                    ...
                    if response.status == 200:
                        ...
                        try:
                            ...
                            result_preview = json.dumps(result, indent=2)[:1000] if result else "None"
                        except:
                            result_preview = str(result)[:1000] if result else "None"
                        print_agent_output(f"Raw response from {endpoint_name}: {result_preview}", agent="TRANSLATOR_DEBUG")
                        
                        # Try multiple possible field structures
                        # 1. Direct fields at root level
                        translated_text = result.get("response") or result.get("output") or result.get("text")
                        ...
                        if not translated_text and isinstance(result, dict):
                            ...
                            # Check for body.output structure (common in webhook responses)
                                ...
                                translated_text = (result["body"].get("output") or 
                        ...
                        if not translated_text and isinstance(result, dict):
                            ...
                            for key, value in result.items():
                                if isinstance(value, str) and len(value) > 1000:  # Likely to be the translation if > 1000 chars
                                    print_agent_output(f"Found potential translation in field '{key}' with {len(value)} chars", agent="TRANSLATOR_DEBUG")
                        ...
                        if translated_text and isinstance(translated_text, str):
                            ...
                            if cleaned_response and cleaned_response not in ["", "...", "…", "N/A", "None"]:
                                ...
                                print_agent_output(f"Translation preview from {endpoint_name}: {preview}", agent="TRANSLATOR")
                                ...
                                english_word_count = sum(1 for word in english_indicators if f" {word} " in first_500_chars)
                                
                                if english_word_count > 10:  # If too many English words found
                                    print_agent_output(f"WARNING: Response from {endpoint_name} appears to still be in English (found {english_word_count} common English words). The endpoint may have returned the original text unchanged.", agent="TRANSLATOR")
                                ...
                                print_agent_output(f"Translation successful from {endpoint_name}: {len(cleaned_response)} chars", agent="TRANSLATOR")
                                ...
                                print_agent_output(f"Endpoint {endpoint_name} returned empty/placeholder: '{cleaned_response}'", agent="TRANSLATOR")
                                return {"success": False, "endpoint_name": endpoint_name, "reason": "empty_response"}
                        else:
                            # Log the actual structure for debugging
                            print_agent_output(f"Endpoint {endpoint_name} returned unexpected structure. Type: {type(translated_text)}, Keys: {result.keys() if isinstance(result, dict) else 'N/A'}", agent="TRANSLATOR")
                            return {"success": False, "endpoint_name": endpoint_name, "reason": "invalid_structure"}
                    else:
                        # Intelligent error handling based on status code and endpoint
                        error_text = await response.text()
                        self._handle_http_error(endpoint_name, response.status, error_text, endpoint_url)
            ...
            if not ("saola-great.ts.net" in endpoint_url or "Backup-2" in endpoint_name):
                self._endpoint_failures[endpoint_name] = self._endpoint_failures.get(endpoint_name, 0) + 1
            print_agent_output(f"Endpoint {endpoint_name} timed out after {translation_timeout} seconds. Consider increasing TRANSLATION_TIMEOUT env var if translations are completing but timing out.", agent="TRANSLATOR")
        except aiohttp.ClientError as e:
            # Count connection errors as failures for non-Backup-2 endpoints
            if not ("saola-great.ts.net" in endpoint_url or "Backup-2" in endpoint_name):
                self._endpoint_failures[endpoint_name] = self._endpoint_failures.get(endpoint_name, 0) + 1
            print_agent_output(f"Endpoint {endpoint_name} connection error: {e}", agent="TRANSLATOR")
        except json.JSONDecodeError as e:
            # JSON decode errors are always counted as failures
            self._endpoint_failures[endpoint_name] = self._endpoint_failures.get(endpoint_name, 0) + 1
            print_agent_output(f"Endpoint {endpoint_name} JSON decode error: {e}", agent="TRANSLATOR")
        except Exception as e:
            # Other unexpected errors are always counted as failures
            self._endpoint_failures[endpoint_name] = self._endpoint_failures.get(endpoint_name, 0) + 1
            print_agent_output(f"Endpoint {endpoint_name} unexpected error: {e}", agent="TRANSLATOR")
        
        return {"success": False, "endpoint_name": endpoint_name}
    
    def _handle_http_error(self, endpoint_name: str, status_code: int, error_text: str, endpoint_url: str) -> None:
        ...
        if not (status_code == 502 and ("saola-great.ts.net" in endpoint_url or "Backup-2" in endpoint_name)):
            self._endpoint_failures[endpoint_name] = self._endpoint_failures.get(endpoint_name, 0) + 1
        ...
        if status_code == 502:
            ...
            if "saola-great.ts.net" in endpoint_url or "Backup-2" in endpoint_name:
                ...
                print_agent_output(f"Endpoint {endpoint_name} service unavailable (502) - may be temporarily down", agent="TRANSLATOR")
        
        elif status_code in [503, 504]:
            # 503 Service Unavailable, 504 Gateway Timeout - temporary issues
            print_agent_output(f"Endpoint {endpoint_name} temporarily unavailable ({status_code})", agent="TRANSLATOR")
        
        elif status_code == 429:
            # Rate limiting
            print_agent_output(f"Endpoint {endpoint_name} rate limited (429) - backing off", agent="TRANSLATOR")
        
        elif status_code in [400, 401, 403]:
            # Client errors that indicate configuration issues
            print_agent_output(f"Endpoint {endpoint_name} client error ({status_code}): {error_text[:100]}", agent="TRANSLATOR")
        
        elif status_code in [500, 501, 505]:
            # Server errors that are unexpected
            print_agent_output(f"Endpoint {endpoint_name} server error ({status_code}): {error_text[:100]}", agent="TRANSLATOR")
        
        else:
            # Other HTTP errors
            print_agent_output(f"Endpoint {endpoint_name} HTTP error ({status_code}): {error_text[:100]}", agent="TRANSLATOR")
    
    def _select_best_translation_result(self, results: List[Dict[str, Any]], original_length: int = None) -> Optional[Dict[str, Any]]:
        """
        ...
        print_agent_output(f"Selecting from {len(results)} translations using priority-based selection:", agent="TRANSLATOR")
        ...
        sorted_results = sorted(clean_results, key=lambda x: x["priority"]) + sorted(warned_results, key=lambda x: x["priority"])
        ...
        for result in sorted_results:
            ...
            print_agent_output(f"  Priority {priority} - {endpoint_name}: {length} chars{warning}", agent="TRANSLATOR")
        ...
        if original_length is None and sorted_results:
            ...
            original_length = int(sum(r.get("length", 0) for r in sorted_results) / len(sorted_results))
            print_agent_output(f"Estimated original length: {original_length} chars", agent="TRANSLATOR")
        ...
        print_agent_output(f"Validation thresholds - Min preferred: {min_preferred}, Min valid: {min_valid}, Max valid: {max_valid}", agent="TRANSLATOR")
        ...
        for result in sorted_results:
            ...
            if min_valid <= length <= max_valid:
                ...
                if length >= min_preferred:
                    print_agent_output(f"Selected Priority {priority} ({endpoint_name}): {length} chars - meets preferred threshold", agent="TRANSLATOR")
                    return result
                else:
                    # Valid but not preferred (70-89%), continue checking higher priority
                    print_agent_output(f"Priority {priority} ({endpoint_name}): {length} chars - valid but below preferred (continuing)", agent="TRANSLATOR")
            else:
                # Outside valid range
                if length < min_valid:
                    print_agent_output(f"Priority {priority} ({endpoint_name}): {length} chars - too short (<70%)", agent="TRANSLATOR")
                else:
                    print_agent_output(f"Priority {priority} ({endpoint_name}): {length} chars - too long (>150%)", agent="TRANSLATOR")
        ...
        for result in sorted_results:
            ...
            if min_valid <= length <= max_valid:
                endpoint_name = result["endpoint"]["name"]
                priority = result["priority"]
                print_agent_output(f"Selected Priority {priority} ({endpoint_name}): {length} chars - first valid result", agent="TRANSLATOR")
                return result
        
        # No valid results found
        print_agent_output("No translation results within valid range (70%-150% of original)", agent="TRANSLATOR")
        return None
