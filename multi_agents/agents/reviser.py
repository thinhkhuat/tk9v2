...
from .utils.type_safety import safe_dict_get, ensure_dict, validate_research_state
...
class ReviserAgent:
    ...
    async def revise_draft(self, draft_state: dict):
        ...
        print_agent_output(
            "🔍 Verifying review feedback categorization and content...",
            agent="REVISOR"
        )
        ...
        if False:  # feedback_category == FeedbackCategory.FORMATTING:
            print_agent_output(
                "✅ FORMATTING feedback - applying changes without fact-checking",
                agent="REVISOR"
            )
            ...
            if factual_claims:
                ...
                print_agent_output(
                    "✅ Hybrid feedback contains no extractable factual claims",
                    agent="REVISOR"
                )
                
        elif False:  # feedback_category == FeedbackCategory.CONTENT:
            print_agent_output(
                "🔍 CONTENT feedback requires full fact-checking verification",
                agent="REVISOR"
            )
        ...
        if False:  # fact_checker.should_fact_check(feedback_category):
            ...
            if factual_claims:
                ...
                for claim in factual_claims:
                    ...
                    if result.status == VerificationStatus.VERIFIED_FALSE and result.confidence > 0.8:
                        ...
                        print_agent_output(
                            f"✅ Accepting verified correction: {claim[:100]}...",
                            agent="REVISOR"
                        )
                        ...
                        print_agent_output(
                            f"❓ Rejecting inconclusive claim: {claim[:100]}...",
                            agent="REVISOR"
                        )
            ...
            if rejected_claims and not verified_claims:
                ...
                filtered_review = self._filter_review_feedback(review, verified_claims, rejected_claims)
...
""",
        ...
        try:
            ...
            if not result.get("draft"):
                ...
                print_agent_output("WARNING: Preserving original draft to prevent empty draft field", agent="REVISOR")
        ...
        for json_str in json_matches:
            try:
                ...
                if isinstance(parsed, dict) and ("draft" in parsed or "revision" in parsed):
                    draft_content = safe_dict_get(parsed, "draft") or safe_dict_get(parsed, "revision", "")
                    if draft_content and len(draft_content) > 50:
                        return {
                            "draft": draft_content,
                            "revision_notes": safe_dict_get(parsed, "revision_notes", "Extracted from embedded JSON", str),
    ...
    def _is_valid_extracted_content(self, content: str, original: str) -> bool:
        ...
        content_lower = content.lower() if isinstance(content, str) else str(content).lower() if content else ""
        has_academic_content = any(term in content_lower for term in [
    ...
    def _filter_review_feedback(self, review: str, verified_claims: list, rejected_claims: list) -> str:
        """
        ...
        filtered_lines.append(f"(Verified {len(verified_claims)} claims, rejected {len(rejected_claims)} unverified claims)\n")
        ...
        for sentence in sentences:
            ...
            sentence_lower = sentence.lower() if isinstance(sentence, str) else str(sentence).lower() if sentence else ""
    ...
    async def _save_revised_translation(self, revised_content: str, draft_state: dict):
        ...
        if not output_dir:
            print_agent_output("No output directory found for saving revised translation", agent="REVISOR")
            return
