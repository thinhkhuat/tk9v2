...
# Import agent classes directly to avoid circular import
...
class ChiefEditorAgent:
    ...
    def _create_output_directory(self):
        ...
        if is_uuid:
            ...
            output_dir = "./outputs/" + sanitize_filename(f"run_{self.task_id}_{query[0:40]}")
    ...
    def _setup_providers(self):
        ...
        try:
            ...
            enhanced_config.config_manager.config = (
                enhanced_config.config_manager._load_config_from_env()
            )
            ...
            try:
                ...
                print_agent_output(
                    "   1. Check your .env file exists and has valid API keys", "PROVIDERS"
                )
                print_agent_output(
                    "   2. Ensure PRIMARY_LLM_PROVIDER and PRIMARY_SEARCH_PROVIDER are set",
                    "PROVIDERS",
                )
                print_agent_output(
                    "   3. Run 'python main.py --config' for detailed diagnostics", "PROVIDERS"
                )

                # Don't fail completely - attempt to continue
                print_agent_output(
                    "Attempting to continue despite configuration issues...", "PROVIDERS"
                )
            ...
            provider_msg = (
                f"Using LLM: {current_providers['llm_provider']}:{current_providers['llm_model']}, "
            ...
            print_agent_output(
                "   3. Set PRIMARY_LLM_PROVIDER (openai/google_gemini/anthropic)", "ERROR"
            )
            print_agent_output(
                "   4. Set PRIMARY_SEARCH_PROVIDER (tavily/brave/google/duckduckgo)", "ERROR"
            )
    ...
    def _initialize_agents(self):
        return {
            "writer": WriterAgent(
                self.websocket, self.stream_output, self.headers, self.draft_manager
            ),
            "editor": EditorAgent(
    ...
    def _add_workflow_edges(self, workflow):
        ...
        workflow.add_conditional_edges(
            "publisher", self._should_translate, {"translate": "translator", "skip": END}
        )
    ...
    def _should_translate(self, research_state: Dict[str, Any]) -> str:
        ...
        if target_lang_str and safe_string_operation(target_lang_str, "lower") != "en":
            print_agent_output(
                f"Translation needed for language: {target_language}", "ORCHESTRATOR"
            )
            return "translate"
        else:
...
- Maintains output quality while eliminating resource waste
    ...
    async def run_research_task(self, task_id=None):
        ...
        try:
            ...
            config = {
                "configurable": {"thread_id": task_id, "thread_ts": datetime.datetime.utcnow()}
            }
            ...
            result = {"task": self.task, "error": error_msg, "workflow_failed": True}
            ...
            if self.draft_manager:
                try:
                    ...
                    print_agent_output(
                        f"Failed to save error state: {save_error}", agent="ORCHESTRATOR"
                    )
    ...
    def _create_workflow_summary(self):
        ...
        summary_content = "# Research Workflow Summary\n\n"
        ...
        for agent, agent_data in agent_outputs.items():
            ...
            summary_content += (
                f"- **{agent.title()}**: {total_outputs} outputs across {len(agent_data)} phases\n"
            )

        summary_content += "\n## Draft Files Organization\n\n"
        summary_content += (
            "All intermediate outputs have been saved in the following structure:\n\n"
        )
        summary_content += "```\n"
