...
class ConfigurationValidator:
    ...
    def __init__(self):
        ...
        self.supported_search_providers = {
            "tavily": {"api_key_env": "TAVILY_API_KEY", "required_env": ["TAVILY_API_KEY"]},
            "brave": {"api_key_env": "BRAVE_API_KEY", "required_env": ["BRAVE_API_KEY"]},
            "google": {
                "api_key_env": "GOOGLE_API_KEY",
                "required_env": ["GOOGLE_API_KEY", "GOOGLE_CSE_ID"],
            },
            "serpapi": {"api_key_env": "SERPAPI_API_KEY", "required_env": ["SERPAPI_API_KEY"]},
            "duckduckgo": {"api_key_env": None, "required_env": []},  # No API key required
        }
        ...
        self.required_core_settings = ["PRIMARY_LLM_PROVIDER", "PRIMARY_SEARCH_PROVIDER"]
        ...
        if not Path(env_path).exists():
            ...
            try:
                ...
                if not content.strip():
                    ...
                    result.add_info(
                        "Environment", f"Environment file '{env_path}' found and readable"
                    )
        ...
        for setting in self.required_core_settings:
            ...
            if not value:
                ...
                result.add_error(
                    ...
                    "Replace placeholder with actual value in .env file",
                )
            else:
    ...
    def validate_search_provider(
        self, provider_key: str = "PRIMARY_SEARCH_PROVIDER"
    ) -> ValidationResult:
        ...
        try:
            ...
            if "language" in task_config:
                ...
                if language not in self.supported_languages:
                    ...
                    result.add_info(
                        "Task Config",
                        f"Language '{language}' ({self.supported_languages[language]}) is supported",
                    )
    ...
    def validate_language_settings(self) -> ValidationResult:
        ...
        if research_language not in self.supported_languages:
            ...
            result.add_info(
                "Language Config", f"Research language set to {lang_name} ({research_language})"
            )
    ...
    def validate_directory_structure(self) -> ValidationResult:
        ...
        if not output_dir.exists():
            ...
            result.add_error(
                "Directory", "outputs directory is not writable", "Check directory permissions"
            )
        else:
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "language": {"research": os.getenv("RESEARCH_LANGUAGE", "en")},
            "directories": {
...
def get_validation_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        "errors": [
            {"component": issue.component, "message": issue.message} for issue in result.issues
        ],
        "warnings": [
            {"component": issue.component, "message": issue.message} for issue in result.warnings
        ],
