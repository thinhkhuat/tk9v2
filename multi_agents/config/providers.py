...
class ProviderConfigManager:
    ...
    def _load_config_from_env(self) -> MultiProviderConfig:
        ...
        primary_llm_model = os.getenv(
            "PRIMARY_LLM_MODEL", self._get_default_model(primary_llm_provider)
        )
        ...
        if os.getenv("FALLBACK_LLM_PROVIDER"):
            ...
            fallback_llm_model = os.getenv(
                "FALLBACK_LLM_MODEL", self._get_default_model(fallback_llm_provider)
            )
    ...
    def get_gpt_researcher_config(
        self, llm_config: Optional[LLMConfig] = None, search_config: Optional[SearchConfig] = None
    ) -> Dict[str, str]:
    ...
    def validate_configuration(self, use_cache: bool = True) -> List[str]:
        ...
        if (
            use_cache
            and self._validation_cache
            and self._last_validation_time
            and current_time - self._last_validation_time < 60
        ):
    ...
    def _basic_validation(self) -> List[str]:
        ...
        if not self.config.primary_llm.api_key:
            issues.append(
                f"Missing API key for primary LLM provider: {self.config.primary_llm.provider.value}"
            )

        # Check primary search
        if (
            not self.config.primary_search.api_key
            and self.config.primary_search.provider != SearchProvider.DUCKDUCKGO
        ):
            issues.append(
                f"Missing API key for primary search provider: {self.config.primary_search.provider.value}"
            )

        # Check fallback providers if configured
        if self.config.fallback_llm and not self.config.fallback_llm.api_key:
            issues.append(
                f"Missing API key for fallback LLM provider: {self.config.fallback_llm.provider.value}"
            )

        if (
            self.config.fallback_search
            and not self.config.fallback_search.api_key
            and self.config.fallback_search.provider != SearchProvider.DUCKDUCKGO
        ):
            issues.append(
                f"Missing API key for fallback search provider: {self.config.fallback_search.provider.value}"
            )
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fallback_search": (
                {
                    "provider": (
                        self.config.fallback_search.provider.value
                        if self.config.fallback_search
                        else None
                    ),
                    "has_api_key": (
                        bool(self.config.fallback_search.api_key)
                        if self.config.fallback_search
                        else False
                    ),
                }
            ...
            "strategies": {"llm": self.config.llm_strategy, "search": self.config.search_strategy},
        }
