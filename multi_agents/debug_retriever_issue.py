...
try:
    ...
    print(
        f"File: {CustomRetriever.__module__.__file__ if hasattr(CustomRetriever.__module__, '__file__') else 'Unknown'}"
    )

    # Try to create an instance with the current environment
    test_retriever = CustomRetriever("test query")
    print("✅ Successfully created original retriever instance")
...
if os.getenv("PRIMARY_SEARCH_PROVIDER") == "brave":
    try:
        ...
        print("✅ Successfully created patched retriever instance")
...
try:
    ...
    researcher = GPTResearcher(query="test query", report_type="research_report", verbose=True)
