...
try:
    from mcp_server import mcp, deep_research, health_check, system_info
...
def lambda_response(status_code: int, body: Dict[str, Any], headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    """
...
async def handle_mcp_request(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if path == '/health' or path.endswith('/health'):
            ...
            if method != 'POST':
                return lambda_response(405, {'error': 'Method not allowed. Use POST for research requests.'})
...
def mcp_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"Processing MCP request: {event.get('httpMethod', 'UNKNOWN')} {event.get('path', 'UNKNOWN')}")
    ...
    try:
        ...
        for record in event.get('Records', []):
            try:
                ...
                logger.info(f"Processing research task: {message_body.get('research_id', 'unknown')}")
    ...
    try:
        ...
        if 'ScheduledEvent' in detail_type:
            ...
            try:
                ...
                # Log health status
                logger.info(f"Scheduled health check result: {health_result.get('status', 'unknown')}")
